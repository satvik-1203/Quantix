...
export default function SubTestItem({ subTest }: { subTest: SubTest }) {
  ...
  const rerunJudge = async () => {
    if (!selectedEmail?.metadata?.threadId) return;
    setRerunLoading(true);
    try {
      const resp = await fetch(`/api/agentmail/judge`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ threadId: selectedEmail.metadata.threadId }),
      });
      if (!resp.ok) {
        const t = await resp.text();
        toast.error("Failed to re-run judge");
        console.error(t);
      } else {
        toast.success("Judge re-run completed");
        // Refresh emails list to reflect updated status/metadata
        fetchEmails();
      }
    } catch (e) {
      toast.error("Error re-running judge");
      console.error(e);
    } finally {
                ...
                <div className="space-y-2">
                  {emails.map((email) => (
                    ...
                    >
                      ...
                      {/* Responder tier & closed reason inline */}
                      <div className="mt-1 flex items-center gap-2 text-[11px] text-muted-foreground">
                        {email.metadata?.experiment?.responderTier && (
                          <span className="px-1.5 py-0.5 bg-muted/60 rounded">
                            Tier: {email.metadata.experiment.responderTier}
                          </span>
                        )}
                        {email.metadata?.closedReason && (
                          <span className="px-1.5 py-0.5 bg-muted/60 rounded">
                            Closed: {email.metadata.closedReason}
                          </span>
                        )}
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between pb-3 border-b">
                    <div className="flex items-center gap-3">
                      ...
                      {selectedEmail?.metadata?.experiment?.responderTier && (
                        <span className="text-xs text-muted-foreground">
                          Tier:{" "}
                          {selectedEmail.metadata.experiment.responderTier}
                        </span>
                      )}
                      {selectedEmail?.metadata?.experiment?.amateurConfidence !=
                        null && (
                        <span className="text-xs text-muted-foreground">
                          Conf:{" "}
                          {Math.round(
                            (selectedEmail.metadata.experiment
                              .amateurConfidence || 0) * 100
                          )}
                          %
                        </span>
                      )}
                  ...
                  {selectedEmail.metadata?.threadId && (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {selectedEmail?.metadata?.closedReason && (
                            <Badge variant="outline" className="text-xs">
                              Closed: {selectedEmail.metadata.closedReason}
                            </Badge>
                          )}
                          <Button
                            size="sm"
                          ...
                          >
                            {rerunLoading ? "Re-running..." : "Re-run Judge"}
                          </Button>
                        </div>
                        {loadingThread && (
                          <span className="text-xs text-muted-foreground">
                  ...
                  {/* Thread Summary (internal) */}
                  {selectedEmail.metadata?.summary && (
